<table class="{{ config('options.styles.table') }}">
	...
	<tbody>
		...
		<tr data-id="{{ $model->id }}" data-tipo="OT">
			...
			<td>
				@if($model->proof_id>0)
				<a href="{{ '/finances/input_vouchers?sn='.$model->proof->sn }}" class="btn btn-link btn-sm" title="Ver OT">{{ $model->proof->sn }}</a>
				@else
				SIN DOC
				@endif
			</td>
			...
			@if($model->status=='APROB')
				<a href="{{ route('input_vouchers.by_order', $model->id) }}" class="btn btn-outline-secondary btn-sm" title="Generar Compra">{!! $icons['invoice'] !!}</a>
			@endif
			@if(in_array($model->status,['CERR', 'ANUL']))
				<a href="{{ route('input_orders.show', $model->id) }}" class="btn btn-outline-secondary btn-sm" title="Ver OC">{!! $icons['view'] !!}</a>
			@else
				<a href="{{ route( 'input_orders.edit' , $model) }}" class="btn btn-outline-primary btn-sm" title="Editar">{!! $icons['edit'] !!}</a>
