<table class="{{ config('options.styles.table') }}">
	...
	<tbody>
		...
		<tr data-id="{{ $model->id }}">
			...
			<td>
				@if($model->order_id == 0)
				<a href="{{ route('orders.by_quote', $model->id) }}" class="btn btn-outline-default btn-sm" title="Generar Pedido">{!! $icons['invoice'] !!}</a>
				@else
				<a href="{{ route('output_quotes.show', $model->id) }}" class="btn btn-outline-default btn-sm" title="Ver Pedido">{!! $icons['invoice'] !!}</a>
				@endif
				@if(1==1)
