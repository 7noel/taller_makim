...
class CompanyRepo extends BaseRepo{
	...
	{
		return [""=>"Seleccionar"] + Company::where('entity_type', 'my_company')->pluck('company_name', 'id')->toArray();
	}
	...
	{
		return Company::where('entity_type', 'employees')->where('my_company', session('my_company')->id)->pluck('company_name', 'id')->toArray();
	}
}